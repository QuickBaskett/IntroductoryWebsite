...
export function Footer() {
  return (
    <footer className="footer-band mx-auto px-[clamp(1.1rem,4vw,2.6rem)] pt-16 pb-8" data-testid="site-footer">
      <div className="grid grid-cols-1 gap-12 border-b border-black/[0.07] pb-12 lg:grid-cols-[1.4fr_2fr]">
        <div>
          ...
          <p className="mt-4 max-w-[40ch] text-[0.9rem] text-fg2">
            Local products, women-led livelihoods and better pauses — from Uttarakhand to the road ahead.
          </p>
        </div>
          ...
          {FOOTER_COLS.map((col) => (
            <div key={col.h}>
              ...
              {col.links.map(([label, href]) => (
                <a key={label} href={href} className="block py-1.5 text-[0.9rem] text-fg2 transition-colors hover:text-acid" data-testid={`footer-link-${label.toLowerCase().replaceAll(' ', '-')}`}>
                  {label}
                </a>
              ))}
            </div>
